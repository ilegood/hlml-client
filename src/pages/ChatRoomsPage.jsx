...
const ChatRoomsPage = () => {
  ...
  const fetchChatRooms = async () => {
    try {
      ...
      const myJoinedRooms = allPosts.filter((post) => {
      ...
      // 강퇴된 방들 마킹
      const myKickedRooms = kickedPosts.map(room => ({ ...room, isKicked: true }));
        ...
        {loading ? (
          ...
          chatRooms.map((room) => (
            <ChatRoomItem 
              key={room.id} 
              room={room} 
              onDelete={room.isKicked ? () => handleDeleteKickedRoom(room.id) : null}
            />
          ))
        ) : (
