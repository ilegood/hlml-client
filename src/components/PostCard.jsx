...
import { STATUS_EMOJI, STATUS_CLASS, getTimeAgo, countComments, formatDateTime } from "../api/homeConstants";
import styles from "./PostCard.module.css";

export default function PostCard({ post, variant = "main", onLike, onOpen, currentUserId }) {
  if (!post) return null;
            ...
            <>
              ...
              {tags.length > 0 && (
                <div className={styles.tagRow}>
                  {tags.map(([, v]) => <span key={v} className={styles.tag}>{v}</span>)}
                </div>
              )}
            </>
          )}
      ...
      <div className={styles.cardFooter}>
        ...
        {variant === "main" ? (
          <div className={styles.capProgressWrap}>
            <div className={styles.capText}>{participants} / {capacity}명 참여중</div>
