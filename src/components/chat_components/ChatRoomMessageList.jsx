...
const ChatRoomMessageList = ({
  ...
  return (
    <div
      className={"[flex:1] [overflow-y:auto] [overflow-x:hidden] [padding:16px_0_8px] [display:flex] [flex-direction:column] [background:var(--color-bg)]"}
      ref={messagesRef}
                ...
                <div
                  className={
                    msg.isDeletionWarning
                      ? "[text-align:center] [font-size:13px] [font-weight:700] [color:#666] [background:#f0f0f0] [padding:8px_16px] [border-radius:8px] [margin:12px_auto] [width:fit-content] [max-width:80%] [border:1px_solid_#ddd]"
                      : "[text-align:center] [font-size:13px] [font-weight:500] [color:#888] [padding:4px_8px] [align-self:center] [margin:4px_0]"
                  }
                >
                  <span>{displaySystemText}</span>
                  {hasMap && (
                    <div className={"[width:100%] [max-width:764px] [margin:8px_auto_0]"}>
                      <MapPreview
          ...
          <div key={msg.id || idx}>
            ...
            >
              <div className={"[width:40px] [flex-shrink:0] [display:flex] [justify-content:center] [padding-top:2px]"}>
                {compact ? (
                  <span className={"[font-size:10px] [color:transparent] [width:40px] [text-align:center] [padding-top:3px] [line-height:1.2]"}>
                    {formatTime(msg.time)}
                  </span>
                ) : (
                  <Avatar
              ...
              <div className={"[flex:1] [min-width:0] [padding-left:12px]"}>
                {!compact && (
                  <div className={"[display:flex] [align-items:baseline] [gap:8px] [margin-bottom:2px]"}>
                    <span
                      className={`${"[font-size:15px] [font-weight:600] [color:var(--color-text)] [cursor:pointer] [line-height:1.3]"} ${isMine ? "[color:var(--color-active)]" : ""}`}
                      onClick={() => setSelectedProfileId(msg.userId)}
                    ...
                    {msgNickname === roomAuthor && (
                      <span className={"[display:inline-block] [background:var(--color-active,_#f39c12)] [color:#fff] [font-size:10px] [font-weight:700] [padding:1px_6px] [border-radius:8px] [margin-left:6px] [vertical-align:middle]"}>방장</span>
                    )}
                ...
                {isMine && msg.readCount > 0 && (
                  <div className={"[font-size:10px] [color:var(--color-active)] [font-weight:700] [margin-top:2px]"}>
                    {msg.readCount}명 읽음
                  </div>
                )}
                  ...
                  <div className={"[display:flex] [flex-wrap:wrap] [gap:4px] [margin-top:4px]"}>
                    {Object.entries(reactionMap).map(
                      ([emoji, { count, mine }]) => (
                        ...
                        >
                          {emoji}
                          <span className={"[font-size:12px] [font-weight:600] [color:var(--color-text)]"}>
                            {count}
                          </span>
                        </div>
                      ),
