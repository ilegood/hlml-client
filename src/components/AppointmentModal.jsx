...
export default function AppointmentModal({ onClose }) {
  ...
  const isSelected = (d) => d === selectedDay.getDate() && viewMonth === selectedDay.getMonth() && viewYear === selectedDay.getFullYear();
      ...
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.calendarContainer}>
          <div className={styles.calHeader}>
            <button className={styles.navBtn} onClick={handlePrevMonth} disabled={isPrevDisabled}>〈</button>
                  ...
                  {hoveredDay === d && hasAppt && (
                    <div className={styles.dotPopup}>
                      {dayAppts.map((appt, idx) => (
                        ...
                        >
                          <span className={styles.dotPopupTime}>{formatTime(appt.time)}</span>
                          <span className={styles.dotPopupTitle}>{appt.title}</span>
                        </div>
                      ))}
                    </div>
                  )}
        ...
        <div className={styles.apptList}>
          ...
          {selectedDateAppts.length > 0 ? (
            ...
            <div className={styles.emptyMsg}>선택한 날짜에 예정된 약속이 없습니다.</div>
          )}
        </div>
